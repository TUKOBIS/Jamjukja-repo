...
def index():
    try:
        ...
        try:
            movie = pd.read_sql(
                "SELECT * FROM movie_tb ORDER BY sales DESC LIMIT 30", con=conn)
...
def result():
    try:
        ...
        try:
            ...
            movie = pd.read_sql(
                "SELECT * FROM movie_tb WHERE title LIKE '%{}%'".format(title), con=conn)
            movie = movie.to_dict('records')[0]

            # 배우, 나라, 감독, 장르, 리뷰
            actor = pd.read_sql("SELECT a.actor FROM movie_actor_tb m JOIN actor_tb a on a.id=m.movie_id WHERE m.movie_id = {} LIMIT 1".format(
                movie.get('id')), con=conn)
            if len(actor) > 0:
                actor = actor.to_dict('records')[0]
            else:
                actor = {"actor": "알 수 없음"}
            country = pd.read_sql("SELECT c.country FROM movie_country_tb m JOIN country_tb c on c.id = m.movie_id WHERE movie_id = {} LIMIT 1".format(
                movie.get('id')), con=conn)
            if len(country) > 0:
                country = country.to_dict('records')[0]
            else:
                country = {"country": "알 수 없음"}
            director = pd.read_sql("SELECT d.director FROM movie_director_tb m JOIN director_tb d on d.id = m.movie_id WHERE movie_id = {} LIMIT 1".format(
                movie.get('id')), con=conn)
            if len(director) > 0:
                director = director.to_dict('records')[0]
            else:
                director = {"director": "알 수 없음"}
            genre = pd.read_sql("SELECT g.genre FROM movie_genre_tb m JOIN genre_tb g on g.id = m.movie_id WHERE movie_id = {} LIMIT 1".format(
                movie.get('id')), con=conn)
            if len(genre) > 0:
                genre = genre.to_dict('records')[0]
            else:
                genre = {"genre": "알 수 없음"}
            review = pd.read_sql("SELECT m.review FROM movie_review_tb m WHERE movie_id = {}".format(
                movie.get('id')), con=conn)
    ...
    try:
        ...
        try:
            movie = pd.read_sql(
                "SELECT * FROM movie_tb ORDER BY sales DESC LIMIT 30", con=conn)
            movie = movie.to_dict('records')
            return render_template('movie_ranking.html', movie=movie,)
        except Exception as e:
            return redirect(url_for('_except', err=e))
        finally:
            conn.close()
